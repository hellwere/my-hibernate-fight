...
public class CacheExercise {
    public void callExercise() {
        ...
        customerService.addCustomers(customers).forEach(System.out::println);
        customerService.findCustomerByName("Алексей").forEach(System.out::println);
        ...
        employeeService.findEmployeeWithMaxSalary().forEach(System.out::println);
        employeeService.findEmployeeWithMinSalary().forEach(System.out::println);
        System.out.println(employeeService.countMonthSalaryExpencies());

        // Поиск с кэшем первого уровня
        companyServiceService.findActivityWithCacheTwoTimes(List.of(1L)).forEach(System.out::println);
        companyServiceService.findActivityWithCacheThreeTimes(List.of(1L)).forEach(System.out::println);

        // Поиск в кэше второго уровня
        companyServiceService.findActivityWithCacheSecondLevel(List.of(1L)).forEach(System.out::println);
    }
}
